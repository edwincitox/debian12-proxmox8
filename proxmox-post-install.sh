...
# Ensure ksmtuned (ksm-control-daemon) is enabled and Optimize according to ram size
...
# Optimize ZFS arc size accoring to memory size
if [ -z "$XS_ZFSARC" ] ; then
    XS_ZFSARC="yes"
fi
...
#!/bin/sh
# NIWDE2580 Remove subscription banner
...
## Pretty MOTD BANNER
  if ! grep -q https "/etc/motd" ; then
    cat << 'EOF' > /etc/motd.new
	   This system is Optimized by: NIWDE2580
EOF
...
echo "# NIWDE2580" > /etc/edwincitox-pve
date >> /etc/edwincitox-pve

## Script Finish
echo -e '\033[1;33m Finished....please restart the system \033[0m'
echo "Optimizations by edwincitox"
